const iconByVariant = {
  galactico: '/assets/sofa-galactico.png',
...
export default function HypometroIcon({ variant = 'ok', size = 32, label = '' }) {
  ...
  return (
    <span className={`hypo-icon ${variant}`} style={{ width: size, height: Math.round(size * 0.62) }} aria-hidden={!label} aria-label={label || undefined}>
      <img src={src} alt="" />
    </span>
  );
}
